...
import { getBinaryNodeChild, jidDecode } from '../WABinary/index.js';
...
const BotDecode = (jid) => {
	...
	if (/:\d+@/.test(jid)) {
		...
		if (decode) return `${decode.user}@${decode.server}`;
	}
	return jid;
}
const jidResolve = async (sock, jid, server = "pn") => {
	if (!jid) return null;
	try {
		if (server.toLowerCase() === "lid") {
			if (!jid.endsWith('@s.whatsapp.net')) return jid;
			return BotDecode(await sock.signalRepository.lidMapping.getLIDForPN(jid));
		}
		if (server.toLowerCase() === "pn") {
			if (!jid.endsWith('@lid')) return jid;
			return BotDecode(await sock.signalRepository.lidMapping.getPNForLID(jid));
		}
	} catch {
		return jid;
	}
}
export function binaryhelper(sock, m) {
	if (!m) return m
	m.PNToLID = async (jid) => jidResolve(sock, jid, "lid")
	m.LIDToPN = async (jid) => jidResolve(sock, jid, "pn")
	return m
}
